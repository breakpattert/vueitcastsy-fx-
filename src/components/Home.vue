...
this.getuserlike();
	...
	checkPhone:function(){	
		if(this.userphone==''){
			this.$router.replace({path: '/news/newslist'});
			
	
     }else{
     	
     		this.getlist();
//		this.$router.replace({path: '/news/newsinfo'});
     }
	},
    ...
    $.ajax({
        ...
        success:function(data){
            if(data.code==10000){
             ...
             that.userlist=data.data.activity_thirdparty_user;
             console.log(that.userlist.mobile);
            if(that.userlist.mobile==''){
            	that.isshow=false;
